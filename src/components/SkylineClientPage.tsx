...
function FloatingInquiriesButton() {
    return (
        <div className="fixed bottom-6 right-4 md:bottom-8 md:right-6 z-[60] group">
            ...
            <a href={SMS_LINK} className={cn("relative flex items-center justify-center h-14 w-14 md:w-auto md:px-5 rounded-full md:gap-3.5 transition-all duration-500 ease-out bg-[#F9F9F7] backdrop-blur-md border border-neutral-200 shadow-[0_8px_30px_rgba(0,0,0,0.12)] hover:scale-105 hover:border-amber-600/50 hover:shadow-[0_0_30px_rgba(217,119,6,0.2)] active:scale-95")} aria-label="Private Client Advisory via SMS">
...
export default function SkylineClientPage() {
    ...
    return (
        <div className="bg-[#F9F9F7] text-neutral-900 min-h-screen overflow-x-hidden font-sans m-0 p-0 selection:bg-amber-100 selection:text-amber-900 relative">
            ...
            <AnimatePresence>
                {showUpdateBanner && (
                    <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -50, scale: 0.95 }} transition={{ duration: 0.4, ease: "easeOut" }} className="fixed top-24 md:top-auto md:bottom-6 left-1/2 -translate-x-1/2 z-[55] w-max max-w-[90vw]">
            ...
            {/* HERO SECTION */}
            <section id="hero" className="relative w-full h-[100svh] overflow-hidden bg-black">
                ...
                <div className="max-w-[1400px] mx-auto">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
                        <div className="relative w-full aspect-[4/3] md:aspect-square bg-neutral-100 overflow-hidden rounded-sm group">
                            <Image src="https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=2500&auto=format&fit=crop" alt="SkylineDB3 Studio Architecture" fill className="object-cover object-center grayscale group-hover:grayscale-0 transition-all duration-700 scale-105 group-hover:scale-100" sizes="(max-width: 768px) 100vw, 50vw" />
                            <div className="absolute inset-4 border border-white/20 z-10 pointer-events-none" />
                        </div>
                        <div className="text-left">
                            <span className="text-amber-800 text-[8px] md:text-[10px] font-bold uppercase tracking-[0.4em] mb-3 md:mb-4 block font-sans">The Studio</span>
                            <h2 className="font-serif text-3xl md:text-5xl lg:text-6xl text-neutral-900 tracking-tight mb-6 md:mb-8">Uncompromising <br />Spatial Engineering.</h2>
                            <p className="text-neutral-500 font-light text-sm md:text-lg leading-relaxed font-sans mb-8 md:mb-12">SkylineDB3 is a highly calibrated collective of spatial engineers, masterplanners, and structural visionaries operating on a global scale. We master the intersection of structural physics and aesthetic form to deliver landmark environments that meet strict developmental and fiscal thresholds.</p>
                            <a href="/about" className="inline-flex items-center justify-center border border-neutral-200 text-neutral-800 px-8 py-4 font-bold text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.25em] hover:border-amber-900 hover:text-amber-900 hover:bg-amber-50/50 transition-all font-sans rounded-sm group">
            ...
            <footer className="py-20 md:py-40 px-5 md:px-6 bg-[#F4F4F2] border-t border-neutral-200 relative z-10 text-center">
                <div className="max-w-2xl mx-auto">
                    ...
                    <p className="text-neutral-500 mb-8 md:mb-12 font-light leading-relaxed text-xs sm:text-sm md:text-lg max-w-lg mx-auto font-sans">Initiate an absolute architectural evaluation. Connect with our private client desk to commission your space.</p>
